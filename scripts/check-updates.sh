...
if [ -n "$BINDGEN_PIN" ] && [ -n "$BINDGEN_LATEST" ] && ver_gt "$BINDGEN_LATEST" "$BINDGEN_PIN"; then
  ...
  echo -e "                ${DIM}READSTAT_REGEN_BINDINGS=1 cargo build -p readstat-sys --features buildtime_bindgen${RESET}"
  echo -e "                ${DIM}cargo test --workspace${RESET}"
  echo -e "              (Windows also: ${DIM}READSTAT_REGEN_BINDINGS=1 cargo build -p readstat-iconv-sys --features buildtime_bindgen${RESET})"
