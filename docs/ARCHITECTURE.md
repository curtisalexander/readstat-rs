...
|----------|-------|------|-------|
| **Windows** (`windows-msvc`) | Static — win-iconv (public domain) compiled by `readstat-iconv-sys` | Static — compiled via `libz-sys` crate | `readstat-iconv-sys` is a `cfg(windows)` dependency; needs `LIBCLANG_PATH` |
