...
pub fn get_row_count(
    ...
    let parser: *mut readstat_sys::readstat_parser_t =
        unsafe { readstat_sys::readstat_parser_init() };
    let set_metadata_handler_error =
        unsafe { readstat_sys::readstat_set_metadata_handler(parser, Some(handle_metadata)) };
    if verbose {
        println!(
    ...
    let error: readstat_sys::readstat_error_t =
        unsafe { readstat_sys::readstat_parse_sas7bdat(parser, psas_path_cstring, pmy_count_void) };
    if verbose {
