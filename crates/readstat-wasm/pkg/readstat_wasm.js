 ...
 * calling `read_metadata` or `read_metadata_fast`.
 */
...
export function read_metadata(bytes) {
  return _callReadMetadata(instance.exports.read_metadata, bytes);
}
...
export function read_metadata_fast(bytes) {
  return _callReadMetadata(instance.exports.read_metadata_fast, bytes);
}

function _callReadMetadata(fn, bytes) {
  if (!instance) {
  ...
  const resultPtr = fn(inputPtr, bytes.length);

  // Free the input buffer (reuse malloc/free from emscripten).
  instance.exports.free(inputPtr);

  if (resultPtr === 0) {
    throw new Error("read_metadata returned null — parsing failed");
  }
