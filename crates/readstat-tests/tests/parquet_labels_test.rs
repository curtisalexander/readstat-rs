...
fn somedata_parquet_has_column_labels() {
    ...
    let tempfile =
        NamedTempFile::new("somedata_labels.parquet").expect("Failed to create temp file");
    ...
    assert!(
        id_field.metadata().contains_key("label"),
        "ID field should have label metadata"
    );
    assert_eq!(id_field.metadata().get("label").unwrap(), "ID Number");

    let gp_field = schema.field_with_name("GP").expect("GP field not found");
    assert!(
        gp_field.metadata().contains_key("label"),
        "GP field should have label metadata"
    );
    assert_eq!(
        gp_field.metadata().get("label").unwrap(),
        "Intervention Group"
    );

    let age_field = schema.field_with_name("AGE").expect("AGE field not found");
    assert!(
        age_field.metadata().contains_key("label"),
        "AGE field should have label metadata"
    );
    assert_eq!(
        age_field.metadata().get("label").unwrap(),
        "Age on Jan 1, 2000"
    );

    let time1_field = schema
        .field_with_name("TIME1")
        .expect("TIME1 field not found");
    assert!(
        time1_field.metadata().contains_key("label"),
        "TIME1 field should have label metadata"
    );
    assert_eq!(time1_field.metadata().get("label").unwrap(), "Baseline");

    let time2_field = schema
        .field_with_name("TIME2")
        .expect("TIME2 field not found");
    assert!(
        time2_field.metadata().contains_key("label"),
        "TIME2 field should have label metadata"
    );
    assert_eq!(time2_field.metadata().get("label").unwrap(), "6 Months");

    let time3_field = schema
        .field_with_name("TIME3")
        .expect("TIME3 field not found");
    assert!(
        time3_field.metadata().contains_key("label"),
        "TIME3 field should have label metadata"
    );
    assert_eq!(time3_field.metadata().get("label").unwrap(), "12 Months");

    let time4_field = schema
        .field_with_name("TIME4")
        .expect("TIME4 field not found");
    assert!(
        time4_field.metadata().contains_key("label"),
        "TIME4 field should have label metadata"
    );
    ...
    assert!(
        status_field.metadata().contains_key("label"),
        "STATUS field should have label metadata"
    );
    assert_eq!(
        status_field.metadata().get("label").unwrap(),
    ...
    assert!(
        !sex_field.metadata().contains_key("label"),
        "SEX field should not have label metadata"
    );

    let gender_field = schema
        .field_with_name("GENDER")
        .expect("GENDER field not found");
    assert!(
        !gender_field.metadata().contains_key("label"),
        "GENDER field should not have label metadata"
    );
...
fn cars_parquet_has_table_label() {
    ...
    let tempfile = NamedTempFile::new("cars_labels.parquet").expect("Failed to create temp file");
    ...
    assert!(
        schema.metadata().contains_key("table_label"),
        "Schema should have table_label metadata"
    );
    assert_eq!(
        schema.metadata().get("table_label").unwrap(),
        "Written by SAS"
    );
...
fn cars_parquet_has_storage_width_metadata() {
    let tempfile = NamedTempFile::new("cars_widths.parquet").expect("Failed to create temp file");
    ...
    let brand_field = schema
        .field_with_name("Brand")
        .expect("Brand field not found");
    let brand_width: usize = brand_field
        .metadata()
        .get("storage_width")
    ...
    let model_field = schema
        .field_with_name("Model")
        .expect("Model field not found");
    let model_width: usize = model_field
        .metadata()
        .get("storage_width")
    ...
    let engine_field = schema
        .field_with_name("EngineSize")
        .expect("EngineSize field not found");
    assert_eq!(
        engine_field.metadata().get("storage_width").unwrap(),
        "8",
...
fn hasmissing_parquet_has_sas_format_metadata() {
    let tempfile =
        NamedTempFile::new("hasmissing_formats.parquet").expect("Failed to create temp file");
...
fn hasmissing_parquet_has_column_labels() {
    ...
    let tempfile =
        NamedTempFile::new("hasmissing_labels.parquet").expect("Failed to create temp file");
    ...
    assert!(
        pre_field.metadata().contains_key("label"),
        "PRE field should have label metadata"
    );
    assert_eq!(pre_field.metadata().get("label").unwrap(), "PRE");

    let month6_field = schema
        .field_with_name("MONTH6")
        .expect("MONTH6 field not found");
    assert!(
        month6_field.metadata().contains_key("label"),
        "MONTH6 field should have label metadata"
    );
